...
The server must use the second branch to sign two txs called tx3 and tx4. Both allow the user to use the second branch to withdraw the money from the smart contract without waiting if they learn the preimage, but tx3 spends the utxo created by tx0 and tx4 spends the utxo created by tx1. The server shares the signatures for tx3 and tx4 with the user. (In theory he could do this now, but I think it actually makes more sense if he does this earlier, when he shared the rest of the data with the user; it cuts down the number of messages which must be passed back and forth by one.)
...
4. Server prepares the swap "smart contract" and a swap invoice, and sends the user the data described in the second paragraph of "How it works", along with signatures for tx3 and tx4
5. User validates the data, signs tx1 and tx2, and shares their signatures with the server
6. Server validates the signatures and funds the smart contract
...
If, for some reason, the preimage given by the server is *not* the right privkey, the user should withdraw their money immediately using tx3 because the swap server didn’t follow the protocol correctly, and is therefore some kind of cheater. They can't steal the user's money, but they should probably be penalized anyway for making the user use the more expensive spending path. If, upon seeing the user withdraw their money via tx3, the swap service initiates a race condition by broadcasting tx1, the user should be ready to broadcast tx4 in case tx1 confirms before tx3.
