...
Since the user can get the money out either using tx3 or tx4, the user does not need to trust the server. As soon as they pay the lightning invoice, they have everything they need to sweep the money. Nothing bad can happen.

However, there is a way to improve this situation: for a cheaper withdrawal tx, it would be great if the user did not have to use the script path, but could use the key path. (In taproot, the key path costs less than the script path in terms of mining fees.) For this reason, the user optimistically *hopes* the preimage given by the server is *also* the private key to the public key which, when tweaked with the user’s pubkey, yields the key required by the keypath. If it is, then the user can add their own privkey to the preimage to derive the backup key, and then simply import that key to their wallet, without the need to withdraw from the swap address immediately.
