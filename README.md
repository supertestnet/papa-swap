...
Since the user can get the money out either using tx3 or tx4, the user does not need to trust the server. As soon as they pay the lightning invoice, they have everything they need to sweep the money. Nothing bad can happen. Only 1 on-chain transaction has happened, and the user has full control of the money in the smart contract, because tx3 and tx4 let them withdraw it regardless of what the server does. So the user alone controls the funds.
